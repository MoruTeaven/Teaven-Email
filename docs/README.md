...
Teaven Email 是一个基于 **Cloudflare Workers** 的**多租户邮件基础设施平台**，为 SaaS 应用提供完整的邮件发送、模板管理、Provider 管理和异步队列处理能力。

### 核心能力

- **多租户隔离**：每个租户独立管理自己的模板、Provider、发件账号和 API Key
- **三种发送通道**：SMTP（MailChannels）、Cloudflare Email Routing、第三方 API（SendGrid/Mailgun/Resend）
- **模板引擎**：基于 Handlebars，支持版本管理、变量提取和模板预览
- **分类路由调度**：可将不同类别的邮件路由到不同 Provider 和发件账号
