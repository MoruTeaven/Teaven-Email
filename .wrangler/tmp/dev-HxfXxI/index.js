...
// .wrangler/tmp/bundle-QdeTw9/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-QdeTw9/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-QdeTw9/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
function getDB(db) {
  return {
    ...
    async getApiKeyByHash(hash) {
      return db.prepare("SELECT * FROM api_keys WHERE api_key_hash = ? AND enabled = 1").bind(hash).first();
    },
    ...
    async getTemplateByCode(userId, templateCode, version) {
      if (version) {
        return db.prepare(
          "SELECT * FROM templates WHERE user_id = ? AND template_code = ? AND version = ? AND enabled = 1"
        ).bind(userId, templateCode, version).first();
      }
      return db.prepare(
...
async function generateApiKey() {
  ...
  const raw2 = `sk_${hex}`;
  const hash = await hashApiKey(raw2);
  const prefix = raw2.substring(0, 10);
  return { raw: raw2, hash, prefix };
}
...
async function sendViaCloudflareEmail(config, params) {
  try {
...
async function sendViaApi(config, params) {
...
async function sendEmail(provider, params) {
  ...
  switch (provider.type) {
    ...
    case "cloudflare_email":
      return sendViaCloudflareEmail(config, params);
    case "api":
      return sendViaApi(config, params);
    default:
...
templateRouter.get("/:code", authMiddleware(["MANAGE_TEMPLATE"]), async (c) => {
  ...
  const version = c.req.query("version") ? parseInt(c.req.query("version")) : void 0;
...
async function triggerWebhooks(env, userId, event, item) {
  const db = getDB(env.DB);
